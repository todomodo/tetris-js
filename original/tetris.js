...
//*************************************************************
// Return a random shape index from the shape table
//*************************************************************
...
//*************************************************************
// Return a random color style from the color table
//*************************************************************
...
//*************************************************************
// Set the CSS class of a board cell at (x, y)
//*************************************************************
function fnPutPixel(x, y, Color) {
    let board = document.getElementById('TRT_ID_BOARD')
    board.rows[y].cells[x].className = Color;
}

//*************************************************************
// Read the CSS class of a board cell at (x, y).
// Returns g_cSolidColor for out-of-bounds coordinates.
//*************************************************************
...
//*************************************************************
// Paint the current shape on the board using the given color.
// Pass g_cNoColor to erase the shape.
//*************************************************************
function fnPaintShape(Color) {
    for (let i = 0; i < g_aShapeTable[g_iShapeIndex][g_iAngle].length; i += 2) {
        let dx = g_aShapeTable[g_iShapeIndex][g_iAngle][i];
        let dy = g_aShapeTable[g_iShapeIndex][g_iAngle][i + 1];
        fnPutPixel(g_ix + dx, g_iy + dy, Color);
    }
}

//*************************************************************
// One game tick: advance the current shape down by one step.
// If blocked, compact completed rows, update score, and
// introduce the next shape. Ends the game if no room.
//*************************************************************
function fnTick() {
    let i = TRT_fnStep(0, 1);	//move the shape one step down
    if (i == 0) {
...
//*************************************************************
// Update the UI elements displaying shapes, lines, score,
// and level counts
//*************************************************************
...
//*************************************************************
// Reset the board, counters, and timer, then start a new game
//*************************************************************
function TRT_fnRestartGame() {
    ...
    //clear the DC
    for (let x = 0; x < g_ixMax; x++) {
        for (let y = 0; y < g_iyMax; y++) {
...
//*************************************************************
// Rotate the current shape. Direction 1 = counter-clockwise,
// otherwise clockwise. Aborts if rotation would cause a
// collision.
//*************************************************************
function TRT_fnRotate(Direction) {
    ...
    //check
    for (let i = 0; i < g_aShapeTable[g_iShapeIndex][iNewAngle].length; i += 2) {
...
//*************************************************************
// Move the current shape by (dx, dy). Returns 1 if the move
// was performed, 0 if blocked by a collision or boundary.
//*************************************************************
function TRT_fnStep(dx, dy) {
    ...
    for (let i = 0; i < g_aShapeTable[g_iShapeIndex][g_iAngle].length; i += 2) {
        let ix = g_ix + g_aShapeTable[g_iShapeIndex][g_iAngle][i] + dx;
        let iy = g_iy + g_aShapeTable[g_iShapeIndex][g_iAngle][i + 1] + dy;
...
//*************************************************************
// Instantly drop the current shape to the lowest valid position
//*************************************************************
function TRT_fnDrop() {
    fnPaintShape(g_cNoColor);

    for (let iyBase = g_iy; iyBase < g_iyMax; iyBase++) {
        for (let i = 0; i < g_aShapeTable[g_iShapeIndex][g_iAngle].length; i += 2) {
            let ix = g_ix + g_aShapeTable[g_iShapeIndex][g_iAngle][i];
            let iy = iyBase + g_aShapeTable[g_iShapeIndex][g_iAngle][i + 1] + 1;
...
//*************************************************************
// Introduce a new shape at the top of the board with a random
// angle, color, and horizontal position. Returns 1 on success,
// 0 if the shape overlaps existing pixels (game over).
//*************************************************************
function fnDropNewShape(iShapeIndex) {
    ...
    let ixMin = g_aShapeTable[g_iShapeIndex][g_iAngle][0];
    let ixMax = g_aShapeTable[g_iShapeIndex][g_iAngle][0];
    let iyMin = g_aShapeTable[g_iShapeIndex][g_iAngle][1];
    let iyMax = g_aShapeTable[g_iShapeIndex][g_iAngle][1];
    for (let i = 0; i < g_aShapeTable[g_iShapeIndex][g_iAngle].length; i += 2) {
    ...
    //randomize x
    let ixShiftRange = (g_ixMax - 1 - g_ixMin - Math.max(0, -1 * ixMin) - ixMax);
...
//*************************************************************
// Check if row y is completely filled. Returns 1 if full, 0
// otherwise.
//*************************************************************
function fnIsFull(y) {
    for (let x = 0; x < g_ixMax; x++) {
...
//*************************************************************
// Shift all rows above iYend down by one, clearing the
// topmost occupied row. Used after removing a complete line.
//*************************************************************
function fnShiftBlock(iYend) {
    for (let y = iYend; y > g_iyTopLine; y--) {
        for (x = 0; x < g_ixMax; x++) {
...
//*************************************************************
// Find and remove completed rows in the area affected by the
// last placed shape. Increases speed at line milestones
// (every 10 lines up to 80).
//*************************************************************
function fnCompact() {
    let iyBottom = g_iy;
    ...
    //
    for (let y = g_iyTopLine; y <= iyBottom; y++) {
        if (fnIsFull(y) == 1) {
            fnShiftBlock(y);
...
//*************************************************************
// Build the game UI: create the board grid (iXmax x iYmax
// cells) and the keypad with controls and score display
//*************************************************************
function TRT_fnRend(iXmax, iYmax) {
    //build the board
    let board = document.getElementById('TRT_ID_BOARD')
    for (let iRow = 0; iRow < iYmax; iRow++) {
        let newRow = board.insertRow(0);
        for (let iCol = 0; iCol < iXmax; iCol++) {
    ...
    //build the keypad
    tb2 = document.getElementById('TRT_ID_KEYPAD');
    //line 11
...
//*************************************************************
// Handle keyboard input: arrow keys for movement/rotation,
// spacebar to start a new game
//*************************************************************
function TRT_fnOnKeyDown(key) {
    switch (key) {
        case 'ArrowDown':
            TRT_fnDropLink(); //drop
            break;
...
//*************************************************************
// Set the text content of a DOM element (cross-browser)
//*************************************************************
...
//*************************************************************
// Keypad click handlers - delegate to core game functions
//*************************************************************
